...
from student.models import StuExam_DB,StuResults_DB,StudentInfo
...
def getToken(request):
    appId = "5c60c00058c64e3e961d2fb90dee49d1"
    appCertificate = "e1ba6aae346c438a8c6e7e2608d1668b"
...
def view_exams_student(request):
    ...
    list_un = []
    l=[]
    matric=StudentInfo.objects.get(user=request.user)
    mat=matric.stream
    ...
    for exam in exams:
        if StuExam_DB.objects.filter(examname=exam.name ,student=request.user).exists():
            ...
            print(examday)
            print(exammonth)
            if int(exammonth) == int(month):
                if int(examday)==int(day):
                    list_un.append(exam)
